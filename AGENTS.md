# Agent Rules for Apache Pekko Management
...
- Discovery mechanism changes (Kubernetes, Marathon, AWS, Consul) must be tested against the target platform or documented if integration testing is unavailable.
- Cluster bootstrap changes must preserve backward compatibility with existing bootstrap protocols.
...
- Run integration tests when changes affect discovery, lease, rolling update, or cluster bootstrap behavior.

```shell
sbt "integrationTestKubernetesApi / test"
sbt "integrationTestKubernetesDns / test"
sbt "integrationTestLocal / test"
```
...
- Integration tests require a running Kubernetes cluster or Docker environment. If unavailable, record in `Tests`.
...
- Discovery and lease implementations must follow the SPI pattern defined in pekko-discovery and pekko-lease.
